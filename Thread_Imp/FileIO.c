...
/* **************************************************** *
 * IMPORT: Out filename, Array of thread information    *
 * PURPOSE: Write out thread information for reader     *
 *          and writer threads that have completed      *
 *          the task                                    *
 * **************************************************** */
void writeOutArray(char* filename, ThreadInfo* outArray, int arrayLength)
{
    int ii = 0;
    FILE* file = NULL;
    ...
    {
        if(outArray[ii].threadType == 'w')
        {
            fprintf(file, "writer-%d has finished writing %d pieces of data from the data_buffer.\n", outArray[ii].TID, outArray[ii].counter);
        }
        else if(outArray[ii].threadType == 'r')
        {
